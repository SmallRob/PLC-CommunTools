...
namespace ZCS_FormUI
{
    public partial class FrmSelectImage : Form
    {
        ...
        {
            string[] nameList = System.Enum.GetNames(typeof(ZCS_FormUI.FontIcons));
            var lst = nameList.ToList();
            lst.Sort();
            foreach (var item in lst)
            {
                ZCS_FormUI.FontIcons icon = (ZCS_FormUI.FontIcons)Enum.Parse(typeof(ZCS_FormUI.FontIcons), item);
                Label lbl = new Label();
                lbl.AutoSize = false;
                ...
                lbl.Text = "       " + item;
                lbl.Image = ZCS_FormUI.FontImages.GetImage(icon, 32, Color.FromArgb(255, 77, 59));
        ...
        void lbl_DoubleClick(object sender, EventArgs e)
        {
            ...
            ZCS_FormUI.FontIcons icon = (ZCS_FormUI.FontIcons)lbl.Tag;
            int intSize = ucTextBoxEx1.InputText.ToInt();
            if (intSize <= 0)
                intSize = 32;
            SelectImage = ZCS_FormUI.FontImages.GetImage(icon, intSize, txtForeColor.BackColor, txtBackcolor.BackColor == Color.White ? Color.Empty : txtBackcolor.BackColor);
